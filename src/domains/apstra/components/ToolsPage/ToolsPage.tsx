...
import ApstraButton from '../../../../components/ApstraButton';
...
export default ToolsPage;  const hand
leIpSearch = async () => {
  ...
  // Add the remaining JSX for IP Search and Blueprint sections
  return (
    <div className="tools-page">
      ...
      <div className={`tools-content ${!isAuthenticated ? 'not-authenticated' : ''}`}>
                    ...
                    {ipSearchResults.map((result, index) => (
                      <tr key={`${result.blueprintId}-${result.interfaceId}-${index}`}>
                        ...
                        <td className="interface-cell">
                          <ApstraButton
                            type="interface"
                            label={formatInterfaceLabel(result.interfaceIfName, result.interfaceIfType, result.interfaceName)}
                            url={result.interfaceUrl}
                          />
                        </td>
                        <td className="ip-cell">
                          <span className="ip-address">{result.ipAddress}</span>
                        </td>
                        <td className="search-result">
                          <button 
                            className="result-details-btn"
        ...
        {/* Blueprint Actions Section */}
        <section className={`blueprint-section ${!isAuthenticated ? 'disabled' : ''}`}>
          <h2>Blueprint Actions {isAuthenticated ? '✅' : '❌'}</h2>
              ...
              <div key={blueprint.id} className="blueprint-card">
                ...
                <div className="blueprint-actions">
                  <button
                    ...
                    disabled={!isAuthenticated || dumpingBlueprintId === blueprint.id}
                    className="blueprint-action-btn dump-btn"
                  >
                    {dumpingBlueprintId === blueprint.id ? (
                      <>
                  ...
                  <button
                    ...
                    className="blueprint-action-btn leafs-btn"
                  >
                    🌿 Get Leafs
                  </button>
                </div>
